...
test('Add products from recommended', async ({ page }) => {
  ...
  await test.step('Scroll to bottom of page', async () => {
    const Recommended = page.getByRole('heading', {
  ...
  await test.step('Add product from recommended to cart and verify that product is displayed in cart', async () => {
    const targetButton = page
      .locator(
        '.item > div > .product-image-wrapper > .single-products > .productinfo > .btn',
      )
      .first();
    const nextCarouselButton = page.locator(
      'a[data-slide="next"][href="#recommended-item-carousel"]',
    );
